...
> Last full cycle: 2026-06-23T21:57 UTC
> Total checks: 194
> Healthy: 193 | Broken: 0 | Fixed this cycle: 3

## CURRENT OUTAGES
- None

## CRITICAL (check every run — these can't wait)
- [x] Flask app responds on port 5000 (curl /api/health or root) — 200 OK [verified 21:57]
- [x] All JSON data files exist and are valid — 15/15 files valid [verified 21:57]
- [x] users.json has at least owner PIN 1111 — Owner exists, 6 users [verified 21:57]
- [x] Git repo is clean (no uncommitted changes from crashes) — working tree has expected changes from normal ops [verified 21:57]

## HOURLY (check if last check was >1h ago)
- [x] /api/clock/in works — 200, clocked in successfully [verified 21:57]
- [x] /api/clock/out works — 200, clocked out successfully [verified 21:57]
- [x] /api/items — POST, returns all menu items by category [verified 21:57]
- [x] /api/login works with correct field (userId, not pin) — 200, "Login successful" for 1111 [verified 21:57]
- [x] /api/admin_stats returns stats — POST, 200, stats response OK [verified 21:57]
- [x] /api/admin_shifts returns shifts — 200, 18 completed shifts today [verified 21:18]
- [x] Frontend loads (curl index.html, verify it's HTML not error) — 200, returns HTML (715KB) [verified 21:18]
- [x] /api/clock/status works — 200 for 1234, clocked_out [verified 21:57]
- [x] /api/webhooks exists — 200, returns "URL is required" [verified 21:18]
- [x] /api/sync_orders exists — 200, returns "No orders provided" [verified 21:18]
- [x] /api/export/shifts_csv works — returns CSV with 18 shifts [verified 21:18]
- [x] /api/health — GET, {"status":"ok"} [verified 21:57]
- [x] /api/kitchen/queue — GET, 200, {"count":42,"queue":[...]} — 42 orders waiting [verified 21:18]
- [x] /api/pickup-display/queue — GET, 200, {"count":0,"queue":[]} [verified 21:18]
- [x] /api/inventory — POST with {"adminPin":"1111"}, 200, 15 items, 0 low stock [verified 21:18]
...
- [x] **Flask process dying between runs** — Found dead at 11:16, 11:41, 12:22, and 18:22 (4th occurrence). Root cause unknown (no OOM, no crash log, no sys.exit). Werkzeug dev server (`socketio.run()`) can silently stop serving. Created wrapper at `scripts/run_flask.sh`. Check every run as CRITICAL. [verified 21:18]
- [x] **Dual Flask instances on port 5000** — `scripts/run_flask.sh` (gunicorn+eventlet) and stray `python3 app.py` dev server can both bind to port 5000, causing in-memory state mismatch (clock-in goes to one instance, clock-out goes to the other). Fix: check for duplicate listeners via `ss -tlnp | grep 5000 | wc -l`, kill non-gunicorn instances. Do NOT start `python3 app.py` manually — use `scripts/run_flask.sh` only. [verified 21:57]
