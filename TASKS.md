...
|- [~] worker-1 **Error state testing** — Test every endpoint with bad data: missing fields, wrong types, oversized values, SQL injection attempts. The app should return clear error messages, not 500 errors or blank screens. Common gaps: no try/catch on JSON.parse in frontend, no 404 handling for deleted items, no fallback when a JSON file is corrupted, no loading states during API calls.
