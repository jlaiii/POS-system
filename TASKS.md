...
- [~] worker-2 **Add order review/confirmation step before final submit** — Currently the "Submit Order" button submits immediately with no confirmation step. Waiters can't review the full order (modifiers, item notes, totals, payment split) before committing. In a real restaurant, this causes costly mistakes that require refunds and re-orders. Implement a 2-step flow: Cart → Review modal showing all items with modifiers, per-item notes, order notes, total, and payment method → Confirm button. Also add a 30-second undo grace period where the order can be voided with one tap after submission, shown as an "Undo (25s...)" button in the toast.

- [ ] **HIGH: Add "Rush Order" / priority flag on order submission** — Waiters need a way to communicate urgency to the kitchen (customer complaints, VIPs, large parties waiting). Currently there's no manual priority override — the kitchen only uses age-based priority (5 min warning, 10 min critical). Add a "🚨 Rush" toggle button on the POS cart (next to Submit Order) that, when enabled before submit, marks the order with a `priority: "rush"` field. The kitchen display then shows a prominent red "🚨 RUSH" badge with pulsing border regardless of age, overriding normal aging colors. Saves waiters from walking to the kitchen to verbally flag urgent orders. Backend: store `priority` field on order records (null or "rush"), kitchen queue endpoint returns it. Frontend: toggle in cart, badge in kitchen card. [System Auditor]

- [ ] **MEDIUM: Add table number quick-filter in order history view** — Waiters frequently need to find orders for a specific table (check what was ordered, re-fire items, check status). Currently the History tab shows all orders chronologically with no table filter. Add a table number input filter above the order history list that filters results client-side to only show orders for the selected table. Combined with the existing date range filter, this saves waiters from scrolling through hundreds of orders to find "Table 5's order from 20 minutes ago." Simple table number dropdown (populated from known table numbers in orders). [System Auditor]
