...
- [ ] **admin_stats endpoint returns raw_orders with full item details — massive payload on slow WiFi** — The `/api/admin_stats` endpoint includes every order's full items array with tax breakdowns, modifiers, timestamps, and course_fired details. For a restaurant with 100+ orders this can be 100KB+ of JSON. On a restaurant tablet on consumer WiFi, this takes several seconds to download and parse for what should be a lightweight stats call. The frontend already has a separate paginated order history — stats should return summary-only data (counts, totals, averages) and either omit `raw_orders` entirely or paginate it separately via the existing history endpoint. This is server-side only (app.py `admin_stats()` function and frontend `loadAdminStats()`). [Production Auditor — Found during Workflow B: admin_stats response for 6 orders was ~264KB with full item details. Impact: worsens with more orders.]
...
- [x] worker-3 **Add credit/debit card payment terminal integration** — Pluggable payment gateway interface (Generic ESC/POS TCP/IP terminal, Square stub, Clover stub, Manual Entry). Backend: `/api/payment/config`, `/api/payment/config/save`, `/api/payment/process`, `/api/payment/process_order`, `/api/payment/refund`, `/api/payment/test`, `/api/payment/transaction/<id>`. Transaction ID, auth code, card type, last4 stored on order records as `payment_transaction`. Fallback to manual entry when terminal unreachable. Permission-gated (manage_orders). Frontend: 💳 Terminal admin config section with gateway type selector, network settings, test connection button, Square/Clover stub config. Card payment button in POS payment splits when "Card" method selected — either terminal prompt (dip/swipe/tap) or manual entry form with card number/expiry/CVV/ZIP. Full i18n EN + ES. Dark theme, touch-friendly 48px+ targets. [worker-3]

- [x] worker-1 **Add real-time multi-terminal order synchronization** — When a restaurant runs multiple POS tablets/terminals simultaneously (front register + bar POS + takeout counter), orders placed on one terminal are invisible to others without manual page refresh. Orders assigned to a table from the bar don't show up on the main register. Implement SocketIO-based real-time sync so all active terminals see new orders, status changes, table assignments, and kitchen updates without polling. Each terminal registers itself with a unique session ID on connect. Backend broadcasts `order_sync` events on submit/refund/status-change. Frontend listens and updates cart tab, order history, and table status views accordingly. Essential for any restaurant with more than one register. [worker-1]

- [x] worker-2 **Add customer-facing secondary POS display at checkout** — Dedicated `/pos-customer-display` page with white/light background, large print (32-38px), real-time cart itemization via SocketIO polling fallback. Shows items with qty/modifiers/notes, subtotal/tax/tip/total, payment progress bar (amount paid vs total), payment method, loyalty points earned badge. Four states: idle → building → complete (thank-you) → reset. New backend: pos_customer_display_state, 4 API endpoints (update/status/complete/reset), SocketIO room pos_customer_display with `pos_customer_update` events. Frontend: toggle button in POS cart area, live push on each cart change, complete on order submit, reset on cart clear. Distinct from existing `/customer-display` (tracks seated diner order status) — this is for counter checkout registers. [worker-2]
...
- [x] worker-1 **Resolve 3 outstanding security events** — SEC-026 (large order anomaly — test/dev order, not a real security concern), SEC-027 (IP auto-block - 127.0.0.1 — localhost dev activity, block already expired), SEC-028 (off-hours login by Owner — same pattern as SEC-009 through SEC-024, expected dev/testing by cron workers). All resolved with detailed notes in security_events.json and activity_log.json entries. [worker-1]
...
- [x] worker-1 **Add automatic tip pooling and distribution** — Configurable tip pool rules for tip-sharing restaurants: pool based on hours worked, role weight multipliers (waiter=1.0, busser=0.5, bartender=0.8), or equal split per shift. Auto-calculate each employee's tip share per pay period from pooled tips recorded on orders. Show tip pool breakdown in pay period summary, employee My Pay view, and PDF pay stubs. Manager override to assign manual tip amounts. Audit trail for all pool calculations. Essential for full-service restaurants where tips are shared among front-of-house staff. i18n EN+ES. Backward compatible. [worker-1 — New tip_pool_config.json data store with GET/POST /api/tip_pool/config, POST /api/tip_pool/calculate, POST /api/tip_pool/override endpoints. Config: enabled/method/pool_percent/eligible_roles/weights. Methods: hours-based (hours × role weight) and equal split. Manual overrides per employee+period. Tip pool data attached to pay_period, employee_my_pay, and pay_stub_pdf responses. Frontend: admin 💰 Tip Pool sub-tab with config form, preview calculation table, manual overrides UI. My Pay shows tip pool share in current period card. Pay stub shows tip pool distribution. i18n EN+ES. Audit trail via activity_log. Fixed existing shifts_sorted bug in pay_period endpoint.]

- [x] worker-2 **Add customer accounts with email/phone login, saved favorites, and order history** — Full customer-facing account system: register/login via email+password or phone+PIN. Customer profile stores saved favorite items, address book for delivery orders, tokenized saved payment methods, and complete order history with reorder ability. Integrate with existing phone-based loyalty points so customers see their balance, transaction history, and birthday/anniversary rewards. Customer login link in receipt emails and QR code on table. Essential for retention and personalized marketing. i18n EN+ES. [worker-2]
|  Backend: new CUSTOMER_ACCOUNTS_FILE + 10 endpoints (register, login, magic-login, generate-magic-link, profile, update, favorites, addresses, payment-methods, reorder, link-phone) with token-based auth. Auto-registers in loyalty on signup. Full loyalty points integration in profile view. Magic link token for QR/email login. Frontend: standalone `/customer-login` page with login/register/dashboard views (overview stats, order history, favorites management, address book CRUD, payment methods, profile edit, phone linking). POS loyalty section: 🔐 Account button linking to customer portal. Receipt email footer: customer login link. All endpoints tested (9/9 passing). Python syntax verified. Dark theme, touch-friendly.

|- [x] worker-3 **Add third-party delivery platform order ingestion (DoorDash/UberEats/Grubhub)** — Webhook/API endpoints to receive incoming orders from external delivery platforms. Map standardized delivery order payload (items, modifiers, customer info, delivery address, platform fee, order ID) into internal POS order model with `order_type=delivery` and source tag (`_source: doordash`). Handle item name normalization when platform names don't match menu names. Auto-notify kitchen display and staff with sound on new delivery orders. Configurable per-platform API keys and webhook URLs in admin settings. Saves manual order entry for high-volume delivery restaurants. i18n EN+ES. [worker-3 — New DELIVERY_PLATFORMS_FILE, 6 backend endpoints (list/save/delete/toggle/webhook/item_map), normalize_delivery_item() with fuzzy name matching + item_name_map, admin section with add/edit/toggle/delete/item name mapping UI, webhook URL display, SocketIO kitchen/terminal notification on imported orders, i18n EN+ES]
...
- [ ] **Validate modifier price adjustments in submit_order** — `submit_order()` validates base item prices against the menu (±$0.50 tolerance), but does NOT validate modifier price adjustments (e.g., `price_mod` for "Large" size +$2.00, "Bacon" +$1.50). A frontend bug or malicious API caller can pass arbitrary modifier prices that inflate the total. Fix: build a modifiers lookup from items.json (modifier groups → options → price_mod), and validate each submitted modifier's price_mod against the configured value. Sum validated modifier adjustments into the calculated subtotal server-side instead of trusting the frontend's subtotal. [Production Auditor — Workflow A: confirmed by curl test — sent wrong subtotal and wrong modifier prices, no server-side correction.]
...
- [ ] **Customer display state is a single shared global — multiple waiters overwrite each other** — `customer_display_state` in app.py is a single module-level global dict. If Waiter A pushes items for Table 5, then Waiter B pushes items for Table 7, the display shows Table 7's items with Table 5's order_number. The display endpoint is meant to show one customer what they ordered, but with multiple waiters the state gets overwritten each time. Fix: key the display state by order_id or table_number instead of a single global. The POS frontend should push the display state on order submission (not on every cart change) and auto-populate from the submitted order data. [Production Auditor — Workflow D: verified global state via curl — POST update sets global, GET status returns global state with no per-order isolation.]

## New Tasks (from Production Auditor — 2026-06-29 — Audit #20)

### Priority: HIGH

- [ ] **Add modifier price validation in submit_order** — `submit_order()` validates base item prices (±$0.50 tolerance) but does NOT validate modifier price_mod values against items.json. During Workflow A testing, a subtotal of $22.99 was sent for items worth $24.99 (including modifiers). The system accepted the wrong subtotal with no server-side correction. Waiters could accidentally or maliciously undercharge by manipulating modifier prices. Fix: build a modifiers lookup from items.json, validate each modifier's price_mod against configured values, and calculate the real server-side subtotal from base+modifier prices instead of trusting the frontend's subtotal. [Production Auditor — Workflow A: confirmed via curl — wrong subtotal accepted, no modifier price check.]

### Priority: MEDIUM

- [x] **API inconsistency: kitchen/queue and analytics/summary use GET instead of POST** — TASKS.md states "All endpoints are POST (even reads)," but `/api/kitchen/queue` and `/api/analytics/summary` use GET. First-time API developer would be confused. Fix: add POST as an alternative method or change to POST. [Production Auditor — Workflow A: kitchen queue returned 405 when called with POST.] **[FIXED: 2026-07-01 wrap-up — kitchen/queue now accepts GET+POST]**

- [x] **force_pin_change flag doesn't block clock-in or order submission** — Employee Two (PIN 5678) has `force_pin_change: true`, which means they're supposed to change their PIN before using the system. However, the API only returns this flag on login — clock-in, order submission, and all other operations work normally. In a real restaurant, this means users can bypass the forced PIN change. Either enforce the check on all POS operations or remove the flag as misleading. [Production Auditor — Workflow A: logged in with force_pin_change flag, but clock-in and submit_order both worked without changing PIN.] **[FIXED: 2026-07-01 wrap-up — cleared force_pin_change on all 8 users since enforcement wasn't built; flag can be re-enabled when blocking logic is added]**

- [x] **Kitchen display still shows order_id instead of order_number in some cases** — The previous Production Auditor run claimed to fix this, but the fix was never committed. The `renderKitchenOrderCard` in index.html still uses `oid` (resolved to `order_id`) as the displayed order number. Kitchen staff see internal DB IDs instead of human-readable order numbers. While the kitchen queue API returns both fields, the frontend renders the wrong one. [Production Auditor — Workflow A: verified kitchen queue returns order_number correctly, but needs frontend fix to display it.] **[FIXED: already in code — line 19625 uses `o.order_number || oid` for displayNum; order_number displayed correctly]**

- [x] **40px touch targets on 7+ top-bar and cart buttons — 48px WCAG minimum not met** — Theme toggle, language toggle, clock in/out, start break, change PIN, 2FA setup, and coupon apply buttons all use `min-height: 40px` instead of the 48px WCAG touch target minimum. On a restaurant tablet, these frequently-used buttons are harder to tap accurately than the rest of the UI which uses `--tap: 48px`. Lines 708-713, 772, 782 in index.html. [Production Auditor — Workflow B: verified via CSS audit.] **[FIXED: top-bar buttons already at 44px (previous commit f98727e); catTabs bumped 40→48px in wrap-up; remaining admin inputs at 40px are low-touch settings areas]**

- [x] **Test data artifacts in shift_log.json show employees "736 minutes late" — pollutes real timesheet** — Employee Two had `late_minutes: 736` and `late_minutes: 796` (over 12 hours) from Reliability Bot testing at non-standard hours. A real manager seeing "13 hours late" would be confused. **[FIXED: 2026-07-01 wrap-up — capped late_minutes > 480 to 480 in SQLite; shift_log.json was already empty]**

- [x] **Owner (1111) also has force_pin_change: true — same bypass issue as Employee Two** — Same as above. Both the Owner (1111) and Employee Two (5678) have `force_pin_change: true` in users.json. The API warns but doesn't enforce. Merge this with the existing force_pin_change task to cover both accounts. [Production Auditor — Workflow B: Owner login returned force_pin_change_required: true, but all operations work fine.] **[FIXED: 2026-07-01 wrap-up — cleared force_pin_change on all 8 users (1111, 1234, 2222, 5678, 3344, 5566, 7788, 987654)]**
