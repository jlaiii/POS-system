...
- [~] worker-3 **Add credit/debit card payment terminal integration** — Currently payment method selection is strictly manual (cash/credit card/other) with zero actual payment processing. A real restaurant must communicate with payment hardware (Clover, Square, Dejavoo, or generic ESC/POS PIN pad) to process card transactions, obtain authorization codes, print customer-facing receipt copies, and capture signatures or PINs. Implement a pluggable payment gateway interface with at least a generic ESC/POS terminal mode (TCP/IP to payment terminal) plus stub/configuration for Square/Clover APIs. Store transaction IDs, authorization codes, and card type on order records. Fallback to "manual entry" mode when terminal is unreachable. Permission-gated.
