...
### Priority: HIGH
- [~] worker-3 **Investigate simultaneous items.json + users.json data corruption** — Both files were replaced with minimal test entries (`{"Test": [{"name": "Item", "price": 5}]}` for items, `{"1111": {"name": "Owner", "role": "owner", "permissions": ["*"]}}` for users) in the same ~40min window. Determine what script or worker wrote this test data. Check all cron workers' recent runs, look for test data generation scripts, and add guards against production file overwrites. Add a git hook or cron check that warns if items.json or users.json shrink below expected size. Consider making critical JSON files read-only (chmod 444) except when explicitly being edited.
