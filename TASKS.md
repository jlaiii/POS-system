...
- [~] worker-3 **HIGH: Waiter "Food Ready" push notification via WebSocket** — When kitchen marks an order as complete (`POST /api/kitchen/complete`), emit a SocketIO event `food_ready` to the waiter who submitted the order (track `waiter_id` on order records). The waiter's POS shows a persistent red badge badge on the History/Orders tab + plays a short alert sound. Currently waiters must manually check order status to know food is ready. One-way notification saves 2-3 trips to the kitchen per shift. WebSocket event also plays `navigator.vibrate(100)` on supported devices for physical confirmation.
