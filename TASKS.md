...
- [~] worker-1 **Add employee performance leaderboard and gamification** — Display a leaderboard in the admin dashboard showing top-performing waitstaff by revenue generated, items sold, average ticket size (with tip %), customer satisfaction rating (from feedback system), and shift punctuality. Pure CSS bar chart comparison with period selector (this week / this month / this pay period). Uses existing data: orders have waiter_id and totals, tickets have user_id, feedback has ratings, timesheet has late/OT tracking. Optional weekly stats summary shown on POS home screen ("You're #2 this week — $45 behind Maria!"). According to restaurant management research, gamification increases average check size by 8–12% and reduces turnover when combined with recognition.
