...
- [~] worker-2 **Add customer-facing secondary POS display at checkout** — For counter-service restaurants with dual-screen setups (customer-facing display at the register), add a dedicated `/pos-customer-display` page showing the current order itemization, running subtotal/tax/total, payment progress, and loyalty points earned — all in large print suitable for reading from 3 feet away. SocketIO-driven so the display stays in sync with the main POS terminal in real time. Clean white-background layout with high-contrast text. Distinct from the existing `/customer-display` (which tracks order status for seated diners) — this is specifically for the checkout counter where the customer watches their order build and reviews before paying.
