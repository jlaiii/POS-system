...
||- [~] worker-2 **MEDIUM: Admin UI for item dietary tags** — The `dietary_tags` field exists on items and is displayed on tablet menu detail popups, but there's NO admin UI to manage them. Add a dietary tag multi-select (🌿 Vegetarian, 🌶️ Spicy, 🥜 Contains Nuts, 🥛 Dairy, 🌾 Gluten, 🥩 Meat, 🐟 Fish, etc.) in both add-item and edit-item forms in the admin panel. Currently dietary_tags is hardcoded to empty array on item creation with no UI to configure it.
