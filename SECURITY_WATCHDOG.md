...
|
|**Activity**: **1 new activity_log entry** since last run (19:09 UTC) — 1 failed login at 19:36 (null user, 127.0.0.1), self-generated by Watchdog test call.
|
|**Login attempts (last 4h)**: 11 total (7 failed, 4 successful), all from 127.0.0.1, all Owner/anonymous. Standard cron worker activity.
|
|**Login attempts (today)**: 82 total (52 failed, 30 successful) — all localhost test traffic.
|
...
|- **Brute force check**: 1 failed login (self-generated test) in this window — well under 5 threshold. Clean.
|- **Account enumeration**: No null-user or non-existent PIN attempts (excluding self-test). Clean.
|- **Successful-after-failure**: No IP with 3+ fails then a success in this window. Clean.
|- **Credential stuffing**: No pattern — single IP 127.0.0.1, single user target. Clean.
|- **Off-hours activity**: Current time 19:37 UTC (2:37 PM CT) — normal business hours. Not flagged.
...
|
|### 📂 File Integrity
|- All JSON files parseable and valid (15 checked).
...
|- No uncommitted data changes to commit (test call recorded in login_attempts.json is expected behavior).
